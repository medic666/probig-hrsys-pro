  ...
  <div>
    <el-card>
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
        ...
        <el-button type="primary" @click="calculateVisible = true">考勤核算</el-button>
      </div>
      <el-table :data="list" v-loading="loading" stripe>
        <el-table-column prop="person_id" label="人员ID" min-width="80" />
        <el-table-column prop="belong_month" label="月份" min-width="100" />
        <el-table-column prop="work_days" label="出勤天数" min-width="90" />
        <el-table-column prop="sick_leave_days" label="病假天数" min-width="90" />
        <el-table-column prop="personal_leave_days" label="事假天数" min-width="90" />
        <el-table-column prop="annual_leave_days" label="年假天数" min-width="90" />
        <el-table-column prop="overtime_workday_hours" label="工作日加班" min-width="100" />
        <el-table-column prop="overtime_holiday_hours" label="节假日加班" min-width="100" />
        <el-table-column prop="violation_count" label="违纪次数" min-width="90" />
        ...
        <el-table-column label="操作" min-width="100">
          <template #default="{ row }">
            <el-button size="small" @click="handleLock(row, !row.is_locked)">
              {{ row.is_locked ? '解锁' : '锁定' }}
            </el-button>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="calculateVisible" title="考勤核算" width="400px">
...
async function fetchData() {
  loading.value = true
  try {
    const res = await api.getAttendanceSummaryList({ page: page.value, page_size: pageSize.value, belong_month: searchForm.belong_month })
...
async function handleCalculate() {
  ...
  try {
    await api.calculateAttendance(calcForm)
...
async function handleLock(row: any, locked: boolean) {
  await api.lockAttendanceSummary({ person_id: row.person_id, belong_month: row.belong_month, is_locked: locked })
