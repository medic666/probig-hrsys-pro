<template>
  <div>
    <div style="display: flex; justify-content: space-between; margin-bottom: 12px">
      ...
      <el-button v-if="auth.hasPermission('organization', 'write')" size="small" type="primary" @click="openCreateEventForm()">新增事件</el-button>
    </div>
    ...
    <el-table :data="events" border size="small" v-loading="loading">
      ...
      <el-table-column prop="company_name" label="组织名称" min-width="120" />
      <el-table-column label="操作" width="120">
        <template #default="{ row }">
          <el-button v-if="auth.hasPermission('organization', 'write')" text size="small" @click="openEditEventForm(row)">编辑</el-button>
          <el-button v-if="auth.hasPermission('organization', 'delete')" text size="small" type="danger" @click="handleDelete(row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
import { listOrganizationEvents, deleteOrganizationEvent } from '../../api/organization'
import type { OrganizationEvent, OrganizationSnapshot } from '../../types'
...
const editSnapshot = ref<OrganizationSnapshot | null>(null)
...
async function fetchData() {
  loading.value = true
  try {
    const res = await listOrganizationEvents({ entity_id: props.entityId, page: page.value, page_size: pageSize })
...
function openEditEventForm(event: OrganizationEvent) {
  editSnapshot.value = event as any
  editEventId.value = event.id
  eventFormVisible.value = true
}

async function handleDelete(row: OrganizationEvent) {
  try {
    await ElMessageBox.confirm('确定删除该事件吗？', '确认删除', { type: 'warning' })
    await deleteOrganizationEvent(row.id)
