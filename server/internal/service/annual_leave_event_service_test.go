...
func TestAnnualLeaveEventListMergesAttendance(t *testing.T) {
	withSalaryDB(t, func(db *gorm.DB) {
		...
		// 全量合并：total=3，按日期倒序
		list, total, err := GetAnnualLeaveEventList(context.Background(), AnnualLeaveListQuery{PageNum: 1, PageSize: 10})
		if err != nil {
			t.Fatalf("list: %v", err)
		}
		if total != 3 {
		...
		list2, total2, _ := GetAnnualLeaveEventList(context.Background(), AnnualLeaveListQuery{PageNum: 1, PageSize: 2})
		if total2 != 3 || len(list2) != 2 {
			t.Fatalf("pagination: total=%d len=%d, want 3/2", total2, len(list2))
		}

		// 人员过滤
		_, total3, _ := GetAnnualLeaveEventList(context.Background(), AnnualLeaveListQuery{PageNum: 1, PageSize: 10, PersonID: 80})
		if total3 != 3 {
			t.Errorf("person filter: total=%d, want 3", total3)
		}
		_, total4, _ := GetAnnualLeaveEventList(context.Background(), AnnualLeaveListQuery{PageNum: 1, PageSize: 10, PersonID: 999})
		if total4 != 0 {
			t.Errorf("person filter none: total=%d, want 0", total4)
		}

		// 类型过滤：grant 只回 account 段
		_, total5, _ := GetAnnualLeaveEventList(context.Background(), AnnualLeaveListQuery{PageNum: 1, PageSize: 10, EventType: "grant"})
		if total5 != 1 {
			t.Errorf("type filter grant: total=%d, want 1", total5)
		}
		// 类型过滤：休假 只回 attendance 段
		_, total6, _ := GetAnnualLeaveEventList(context.Background(), AnnualLeaveListQuery{PageNum: 1, PageSize: 10, EventType: "休假"})
		if total6 != 1 {
			t.Errorf("type filter 休假: total=%d, want 1", total6)
		}

		// 日期范围过滤
		_, total7, _ := GetAnnualLeaveEventList(context.Background(), AnnualLeaveListQuery{PageNum: 1, PageSize: 10, DateStart: "2026-06-01", DateEnd: "2026-06-30"})
		if total7 != 1 {
