...
const isEdit = computed(() => props.person?.id != null)
...
onMounted(async () => {
  ...
  if (isEdit.value) {
    try {
      fillForm((await getPerson(props.person.id)) as any)
    } catch { /* handled */ }
  }
})
...
async function doSave() {
  ...
  saving.value = true
  try {
    const d = await upsertPersonProfile({
    ...
    emit('saved', d?.id || props.person?.id)
  } catch {
    /* handled */
  } finally {
    saving.value = false
