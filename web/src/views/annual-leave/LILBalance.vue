<template>
  <div class="page-container">
          ...
          <PersonCard :person="item" @click="openDetail(item.person_id, item.name)">
            <template #extra>
              <div class="balance-line" :class="{ 'is-zero': !(balanceMap[item.person_id] ?? 0) }">
                调休 {{ hoursToDays(balanceMap[item.person_id] ?? 0).toFixed(2) }} 天
              </div>
            </template>
          </PersonCard>
        </template>
    ...
    <el-dialog v-model="detailVisible" :title="detailPersonName ? `${detailPersonName} 的假期明细` : '假期明细'" width="440px">
      <LeaveBalanceDetail :person-id="detailPersonId" :show-al="false" />
    </el-dialog>
...
async function fetchCards() {
  ...
  const d = (await request.get('/lil-balances', { params: { pageNum: 1, pageSize: 100 } })) as any
