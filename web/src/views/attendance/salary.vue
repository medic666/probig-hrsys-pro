...
import { listMonthlySalary, calcMonthlySalary } from '@/api/attendance'
import type { MonthlySalaryListParams } from '@/api/attendance'
...
const columns = [
  ...
  { prop: 'overtime_workday_salary', label: '工作日加班工资' },
  { prop: 'overtime_holiday_salary', label: '节假日加班工资' },
  { prop: 'attendance_bonus', label: '全勤奖' },
  { slot: 'status', label: '状态', width: 120 }
]

async function fetchList(params: Record<string, unknown>) {
  return listMonthlySalary(params as unknown as MonthlySalaryListParams)
}

async function handleCalc() {
  calcLoading.value = true
  try {
    await calcMonthlySalary({ belong_month: formatDate(new Date()).slice(0, 7) })
